...
e.g., `sdf_crosstab()`, `FactoMineR::CA()`
]
...
.large[
1. **centering and scaling numerical values**  
e.g., `scaled_age = (age - mean(age)) / sd(age)`

<br>

2. **performing string manipulation to extract meaningful variables**  
...
1. Create a pipeline model with decision tree.
...
1. Create a pipeline model with k-means model with 3 clusters.
1. Fit model with data.
